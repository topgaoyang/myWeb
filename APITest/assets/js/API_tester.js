...
function addNewParameter()
{
	...
	$lineContainer.attr({"style":"margin-top:10px","class":"newLine"});
	var $parameterName=$(document.createElement("input"));
	$parameterName.attr({"class":"parameterName","value":$("#paramterName").val()}).appendTo($lineContainer);
	var $parameterValue=$(document.createElement("input"));
	$parameterValue.attr({"class":"parameterValue","value":$("#paramterValue").val(),"style":"margin-left:55px"}).appendTo($lineContainer);
	$lineContainer.insertBefore("#inputLine");
	$("#paramterName").val("");
	$("#paramterValue").val("");
}
function submitForm()
{
	if($("#method2").is(":hidden")){
		var paremtersArray={
			urlHeader:$("#urlHeader").val(),
			parameter:{}
		};
		var urlShow="";
		$("#paramtersContainer").find(".newLine").each(function(index){
			paremtersArray.parameter[$(this).find(".parameterName").val()]=$(this).find(".parameterValue").val();
			temp="&"+$(this).find(".parameterName").val()+"="+$(this).find(".parameterValue").val();
			urlShow=urlShow+temp;
		});
		urlShow=$("#urlHeader").val()+"?"+urlShow.substring(1);;
		$.post("action/apiOperation.php/",paremtersArray,
		 function(data){
		 	$("#RawJson").val(data);
		 	$("#urlContent").text(urlShow);
		 	Process();
		 });
	}
	else{
		var array=$("#fullURL").val().split("?");
		...
		$.each(parameters,function(i,value){
			paremtersArray.parameter[value.split("=")[0]]=value.split("=")[1];
			temp="&"+value;
			urlShow=urlShow+temp;
		});
		urlShow=array[0]+"?"+urlShow.substring(1);
		$.post("action/apiOperation.php/",paremtersArray,
		 function(data){
		 	$("#RawJson").val(data);
		 	$("#urlContent").text(urlShow);
